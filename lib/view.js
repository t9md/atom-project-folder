...
module.exports = class View extends SelectListView {
  ...
  viewForItem(item) {
    ...
    const basename = isGroup ? name : _path.basename(name)
    ...
    return $$(function() {
      ...
      this.li({class: "two-lines"}, () => {
        ...
        this.div({class: "secondary-line path no-icon"}, () =>
  ...
  // -------------------------
  add(...dirs) {
    dirs.filter(isDirectorySync).forEach(dir => {
  ...
  remove(...dirs) {
    for (const dir of dirs) {
      ...
      if (settings.get("closeItemsForRemovedProject")) {
        const dirPrefix = dir + _path.sep
        atom.workspace
          .getTextEditors()
          .filter(editor => editor.getPath().startsWith(dirPrefix))
  ...
  loadConfig() {
    ...
    if (config.groups != null) this.setGroups(config.groups)
  }
